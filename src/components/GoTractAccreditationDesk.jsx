...
const GoTractAccreditationDesk = () => {
  ...
  const exportToExcel = async () => {
    setExporting(true);
    try {
      // Fetch all accredited participants (you might need to adjust this to get all pages)
      const response = await api.get("/gotract/accreditation/accredited", { 
        params: { per_page: 1000 } // Adjust as needed or implement pagination loop
      });
      
      const allAccredited = Array.isArray(response.data?.data) ? response.data.data : [];
      ...
      const excelData = allAccredited.map((p, index) => ({
        ...
        'Accredited Date': p.accreditedAt ? new Date(p.accreditedAt).toLocaleDateString() : 'N/A',
      }));
      ...
      const colWidths = [
        ...
        { wch: 12 }, // Gender
        { wch: 18 }, // Badge Serial
        { wch: 18 }, // Accredited Date
      ];
      ws['!cols'] = colWidths;

      // Add summary information as a separate sheet
      const summaryData = [
        { 'Metric': 'Total Accredited Participants', 'Value': allAccredited.length },
        { 'Metric': 'Export Date', 'Value': new Date().toLocaleString() },
        { 'Metric': 'Report Generated By', 'Value': 'GoTRACT Accreditation System' },
      ];
      
      // Add LGA breakdown if available
      if (stats?.lgas) {
        stats.lgas.forEach(lga => {
          summaryData.push({ 
            'Metric': `LGA: ${lga.lga}`, 
            'Value': `${lga.accredited} / ${lga.registered} accredited` 
          });
        });
      }
      ...
      XLSX.writeFile(wb, `Accredited_Participants_${new Date().toISOString().split('T')[0]}.xlsx`);
